...
export const MyProfile = () => {
    ...
    useEffect(() => {
        try {
            ...
            console.log(token)
            fetch(`https://quizzer-react-node-js-app-dz4a.vercel.app/api/users/profile/${email}`, {
                headers: {
    ...
    const unfollow = () => {
        const token = localStorage.getItem('accessToken')
        try {
            fetch(`https://quizzer-react-node-js-app-dz4a.vercel.app/api/users/${email}/unfollow`, {
                headers: {
                    'Content-type': 'application/json; charset=UTF-8',
                    'token': token
                },
